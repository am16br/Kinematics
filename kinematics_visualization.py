...
t = sp.symbols('t')                             # Define symbolic variable t
# [x(t), y(t), z(t)] position as function of time
#Rate = [sp.sin(3 * t), sp.cos(t), sp.cos(2 * t)]
Rate = [sp.cos(t), sp.sin(t), t / 5]  # Helix: z=t
#Rate = [sp.cos(t), sp.sin(t), 0 * t]  # Circle: z=0*t

Velocity = vector_derivative(Rate, t)                     # velocity = dR/dt
Acceleration = vector_derivative(Velocity, t)             # acceleration = dV/dt
# tangential acceleration = dV/dt (derivative of the magnitude of velocity)
At = vector_magnitude(Velocity).diff(t)

# ======================================= propgate system through time
propagation_time_history = []
for ti in time:
    ti_r = d2r(ti)
    r, r_mag = evaluate_vector(Rate, ti_r)              # evaluate position
    v, v_mag = evaluate_vector(Velocity, ti_r)          # evaluate velocity
    a, a_mag = evaluate_vector(Acceleration, ti_r)      # evaluate acceleration
    v_theta = [r2d(angle) for angle in direction_angles(v, v_mag)]# velocity direction angles
    a_theta = [r2d(angle) for angle in direction_angles(a, a_mag)]# acceleration direction angles
    ut = unit_vector(from_vector_and_magnitude=(v, v_mag))      # unit vector tanjent to trajectory
    ub = unit_vector(from_othogonal_vectors=(v, a))             # unit binormal
    un = unit_vector(from_orthogonal_unit_vectors=(ub, ut))     # unit normal
    at = float(At.subs(t, ti_r).evalf())        # tangential acceleration magnitude
    an = np.dot(a, un)                          # normal acceleration
    rho = v_mag ** 2 / an                       # radius of curvature
    rc = r + (rho * un)                         # position vector of the center of curvature
    rc_mag = vector_magnitude(rc)# magnitude of the position vector of the center of curvature
...
class Animator:
    def __init__(self, simulation_results):
        ...
        self.pos_text = self.text_artist_3d('P@t', 'g')
        self.coc_text = self.text_artist_3d(' c', 'm')
        self.vel_text = self.text_artist_3d('velocity', 'r')
        self.un_text = self.text_artist_3d('UnitNorm', 'k')
        self.ub_text = self.text_artist_3d('UnitBiNorm', 'k')
        self.ut_text = self.text_artist_3d('UVtanTraj', 'k')
